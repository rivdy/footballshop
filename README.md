...
## 6. Perintah Git (add–commit–push)

```bash
git add .
git commit -m "T3: auth (register/login/logout), cookie last_login, restrict views, user-owned data, filter All/My"
git push origin master
# (jika PWS dipakai)
git push pws master
```

**Tips commit message**: jelas, ringkas, formal.
Contoh tambahan:

* `Fix NoReverseMatch: rename url 'home' -> 'show_products'`
* `Add @login_required to main & detail`
* `Set last_login cookie on login; delete on logout`
...
* Halaman *main* & detail **hanya bisa diakses setelah login**.
* Cookie **`last_login`** tersimpan saat login & tampil di halaman utama.
* Data (produk/artikel) otomatis terkait **user** pembuat.
* Tersedia filter **All** vs **My**.
* (Opsional) Functional test Selenium berjalan.
